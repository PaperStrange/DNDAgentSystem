...
| 账户系统 | 注册/登录账号（密码加密哈希存于房主电脑），单点登录：同一账号在新IP登录会挤掉旧会话 |
...
- **账号防冒用**：注册/登录后持有随机秘密重连令牌（仅经 `s:hello` 发给本人，任何快照不包含令牌），伪造 pid 无法劫持他人身份或房主权限；密码经 scrypt 哈希仅存于房主电脑，绝不落日志。
- **单点登录**：同一账号同时只有一条有效会话，新位置登录会挤掉旧连接（收到提示并断开）。
