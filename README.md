...
| 战斗新规 | 首场战斗玩家先动（队内按敏捷高低）；无突袭时比较双方最高敏捷，高者先动；未被怪物发现时先手攻击=突袭。怪物带视野（绿=未暴露/橙=察觉/红=已暴露，暴露立即开战），冒险状态下除BOSS外怪物随机游荡；BOSS视野无限，开战前全队表决（可逃跑，概率成功传送回营地） |
| 营地与短休 | 短休不再瞬间回血：进入营地界面（篝火围坐/恢复生命/购买商品[商人概率刷新]/回到冒险，位置保留），战斗中无法短休 |
| AI DM 调校 | 开局前与每章切换时，AI DM 严格按规则书依据队伍人数/等级/上一章表现调整怪物HP/伤害/数量（数值钳制+离线公式兜底）；NPC对话每次冒险生成变体（线索/奖励不变仅换措辞） |
| 状态与事件树 | 玩家/团队状态机（战斗中/冒险中/buff/debuff 实时展示）；竖状行动区（回合数+全员头像，点击头像查看本次战斗事件树）；怪物/NPC头顶名称+血条+蓝条 |
| 手动战斗 | 手动模式下玩家回合不会超时自动结束（点击按钮确认后才进入下一顺位）；敌方回合需确认推进；不展示快捷键提示 |
| 大厅在线人数 | 大厅实时展示当前在线人数（同一局域网 Unique IP 数，5秒自动刷新）；藏书室与账号绑定（未登录展示为空） |
...
node tools/tuning-probe.mjs     # AI DM难度调校（F-22/F-32）：公式/钳制/快照参数/NPC对话变体
node tools/stealth-probe.mjs    # 视野暴露/怪物游荡/BOSS表决/营地/事件树/先攻（F-23~F-31）
```
