...
export class Rooms {
  ...
  _removeMember(room, pid, { byKick = false, player = null } = {}) {
    ...
    room.charIds?.delete(pid);   // R1-33 Part 2：解除该玩家与角色的绑定（解冲突：从 leaveRoom/kickRoom 的内联块下沉至此，避免被 R1-31 的抽取覆盖）
  ...
  // 解冲突：两卡各自新增「第 5 参」，合并为 6 参（顺序 getPlayer, getAccount；index.mjs 调用点已同步）。
  bindRegistry(getName, isOnline, broadcast, onlineCount, getPlayer, getAccount) { this._registryName = getName; this._isOnline = isOnline; this._broadcast = broadcast; this._onlineCount = onlineCount; this._registryGetPlayer = getPlayer; this._getAccount = getAccount; }
